...
import static com.gtnewhorizon.structurelib.client.world.StructureWorld.PLACE_POSITION;
...
import com.gtnewhorizon.structurelib.client.gui.GuiText;
import com.gtnewhorizon.structurelib.client.gui.TooltipButton;
import com.gtnewhorizon.structurelib.client.renderer.ImmediateWorldSceneRenderer;
import com.gtnewhorizon.structurelib.client.renderer.WorldSceneRenderer;
import com.gtnewhorizon.structurelib.client.world.CreativeItemSource;
import com.gtnewhorizon.structurelib.client.world.StructureFakePlayer;
...
public abstract class GUI_MultiblockHandler<T> {
    ...
    protected static final Vector3i selectedBlock = new Vector3i(0, -1, 0);
    ...
    private void setNextLayer(int newLayer) {
        layerIndex = newLayer;
        if (renderer != null) {
            StructureWorld world = renderer.world;
            resetCenter();
            renderer.clearRenderedBlocks();
            int minY = (int) world.getMinPos().y;
            if (newLayer == -1) {
                renderer.addRenderedBlocks(world.placedBlocks);
                renderer.setRenderAllFaces(false);
            } else {
                world.placedBlocks.longStream().filter(pos -> CoordinatePacker.unpackY(pos) - minY == newLayer)
    ...
    private void resetCenter() {
        StructureWorld world = renderer.world;
        org.joml.Vector3f size = world.getSize();
    ...
    public void drawMultiblock() {
        ...
        if (insideView) {
            ...
            if (middleClickHeld) {
                ...
                Vector3f lookDir = lookAt.sub(eyePos, new Vector3f());
                Vector3f rightDir = lookDir.cross(worldUp, new Vector3f());
                rightDir.normalize();
                Vector3f upDir = rightDir.cross(lookDir, new Vector3f());
                upDir.normalize();
    ...
    protected void initializeSceneRenderer(boolean resetCamera) {
        ...
        if (!resetCamera) {
            try {
                ...
                StructureLib.LOGGER.error("please reset camera on your first renderer call!");
            }
        }

        renderer = new ImmediateWorldSceneRenderer(new StructureWorld());
        renderer.world.updateEntitiesForNEI();
        renderer.setClearColor(guiColorBg);

        renderer.world.placeMultiBlock(stackForm);

        Vector3f size = renderer.world.getSize();
        Vector3f minPos = renderer.world.getMinPos();
        center = new Vector3f(minPos.x + size.x / 2, minPos.y + size.y / 2, minPos.z + size.z / 2);

        renderer.getRenderedBlocks().clear();
        renderer.addRenderedBlocks(renderer.world.placedBlocks);
        renderer.setOnLookingAt(ray -> {});

        renderer.setOnWorldRender(this::onRendererRender);

        blocksBelowController = MathHelper.floor_double(PLACE_POSITION.y - minPos.y);
        selectedBlock.set(0, -1, 0);
        onBlockSelected();
        setNextLayer(layerIndex);
    ...
    private final Vector3i lastLookPos = new Vector3i(0, -1, 0);

    public void onRendererRender(WorldSceneRenderer renderer) {
        final MovingObjectPosition lookingPos = renderer.getLastTraceResult();
        if (lookingPos != null) {
            lastLookPos.set(lookingPos.blockX, lookingPos.blockY, lookingPos.blockZ);
        } else {
            lastLookPos.set(0, -1, 0);
        }

        if (lastLookPos.y >= 0 && lastLookPos.equals(selectedBlock)) {
            renderBlockOverLay(selectedBlock, Blocks.glass.getIcon(0, 6));
            return;
        }
        renderBlockOverLay(lastLookPos, Blocks.stained_glass.getIcon(0, 7));
    ...
    protected void neiOverlay() {
        if (!StructureLib.isNEELoaded) return;
    ...
    private void scanIngredients() {
        ...
        Vector3i renderedBlock = new Vector3i();
        for (final long longPos : renderer.getRenderedBlocks()) {
            CoordinatePacker.unpack(longPos, renderedBlock);
    ...
    private void renderBlockOverLay(Vector3i pos, IIcon icon) {
        if (pos.y < 0) return;
    ...
    public boolean mouseClicked(int button) {
        ...
        if (button == 1 && renderer != null) {
            ...
            selectedBlock.set(lookingPos.blockX, lookingPos.blockY, lookingPos.blockZ);
            onBlockSelected();
        }
        return false;
    }
    ...
    protected void scanCandidates() {
        ...
        final StructureFakePlayer builder = renderer.world.getFakeMultiblockBuilder();
        for (PositionedIStructureElement structureElement : structureElements) {
            if (structureElement.x == selectedBlock.x && structureElement.y == selectedBlock.y
                ...
                IStructureElement.BlocksToPlace blocksToPlace = structureElement.element.getBlocksToPlace(
                        ...
                        AutoPlaceEnvironment.fromLegacy(CreativeItemSource.instance, builder, iChatComponent -> {}));
                if (blocksToPlace != null) {
                    ...
                    for (ItemStack rawCandidate : rawCandidates) {
                        ...
                        for (List<ItemStack> stackedCandidate : stackedCandidates) {
                            List<String> firstCandidateTooltip = stackedCandidate.get(0).getTooltip(builder, false);
                            List<String> rawCandidateTooltip = rawCandidate.getTooltip(builder, false);
