...
public abstract class MultiblockHandler extends TemplateRecipeHandler {
    ...
    public String getGuiTexture() {
        return "structurelib:textures/void.png";
    }
        ...
        public void setResults(List<List<ItemStack>> results) {
            ...
            for (int i = 0; i < results.size(); i++) {
                PositionedStack result = new PositionedStack(
                        results.get(i),
                        CANDIDATE_SLOTS_X - (columnCount - 1) * GUI_MultiblockHandler.SLOT_SIZE
                                + (i / realCandidateInColumn) * GUI_MultiblockHandler.SLOT_SIZE,
                        CANDIDATE_SLOTS_Y + (i % realCandidateInColumn) * GUI_MultiblockHandler.SLOT_SIZE);
