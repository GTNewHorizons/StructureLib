...
public class RenderHelper {
    ...
    final static double BLOCK_DELTA = 0.005;

    public static void drawCuboidSurface(Vector3f zero, Vector3f size, int sides, float red, float green, float blue,
            float alpha) {
        vecZero.set(zero.x - BLOCK_DELTA, zero.y - BLOCK_DELTA, zero.z - BLOCK_DELTA);
        vecSize.set(size.x + BLOCK_DELTA, size.y + BLOCK_DELTA, size.z + BLOCK_DELTA);
