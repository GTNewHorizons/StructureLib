 ...
 * <p>
 * You will construct this definition once per multi, either at class load or at first call to your variant of {@code getStructureDefinition()}.
 * You can use a {@link ClassValue ClassValue&lt;IStructureDefinition&lt;?>>} if you have a template for subclass.
 * You can also use {@link StructureUtility#defer(Function) defer()} or its overloads for simpler cases, but it is
 * slightly less performant as it will allocate new objects on each structure check, whereas {@link ClassValue} will
 * allocate once and cache it onwards.
 * <p>
 * <h2>Constructing</h2>
 * <p>
 ...
 * <ul>
 * <li>{@link com.gtnewhorizon.structurelib.structure.StructureDefinition.Builder#addShape(String, String[][]) addShape()}
 * is a 2d string array describing the layout of structure. You can optionally use `StructureUtility#transpose()`
 * to transpose the array to make the structure look nicer in source code.
 * <li>{@link com.gtnewhorizon.structurelib.structure.StructureDefinition.Builder#addElement(Character, IStructureElement) addElement()}. It takes a char identifier (mapped to a char in any of {@link com.gtnewhorizon.structurelib.structure.StructureDefinition.Builder#addShape(String, String[][]) addShape()} calls) and a {@link IStructureElement}.
 * </ul>
 ...
 * <ul>
 * <li>The "A" position would be the number of blocks on the left side of the controller, not counting controller itself.</li>
 * <li>The "B" position would be the number of blocks on the top side of the controller, not counting controller itself.</li>
...
public interface IStructureDefinition<T> {
    ...
    /**
     * Call {@link #build(Object, ItemStack, String, World, ExtendedFacing, int, int, int, int, int, int)} if {@code hintsOnly},
     * call {@link #hints(Object, ItemStack, String, World, ExtendedFacing, int, int, int, int, int, int)} otherwise.
     *
     * @param object context object. usually multiblock controller.
     * @param trigger The trigger item that contains channel data.
     * @param piece the structure piece's string identifier.
     ...
     * @param basePositionB see class javadoc
     * @param basePositionC see class javadoc
     * @param hintsOnly whether to spawn hints or do creative build. no effect if {@code checkBlocksIfNotNullForceCheckAllIfTrue} is not null.
     * @param checkBlocksIfNotNullForceCheckAllIfTrue creative build or spawn hints if null. Force check not loaded chunk
     *                                                if true. Check only loaded chunk is false.
