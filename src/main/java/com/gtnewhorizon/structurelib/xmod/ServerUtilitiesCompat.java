...
public class ServerUtilitiesCompat {
    ...
    public static boolean checkPermission(World world, EntityPlayer actor, int x, int z) {
        final ForgeTeam owningTeam = ClaimedChunks.instance.getChunkTeam(new ChunkDimPos(x >> 4, z >> 4, world.provider.dimensionId));
        if (owningTeam == null) {
            return true;
        }
        final ForgePlayer currentPlayer = new ForgePlayer(Universe.get(), actor.getUniqueID(), actor.getCommandSenderName());
